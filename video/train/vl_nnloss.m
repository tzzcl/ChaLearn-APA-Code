	...
	if ~opts.noRoot
	  if isempty(dzdy)
		...
		elseif p < 1
		  ...
		  Y = bsxfun(@times, bsxfun(@times, dzdy, y1), max(abs(d), opts.epsilon).^(p-1) .* sign(d)) ;
		else
		  Y = max(sum(abs(d).^p,3), opts.epsilon).^((1-p)/p) ;
		  Y = bsxfun(@times, bsxfun(@times, dzdy, y1), abs(d).^(p-1) .* sign(d)) ;
		end
	  end
	else
	  if isempty(dzdy)
