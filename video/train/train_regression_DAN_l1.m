function [net, info] = train_regression_DAN_l1(varargin)
opt.model = 'vgg-face';
...
net.addLayer('concat',dagnn.Concat,{'x32_max_norm','x32_avg_norm'},'x32_concat');
fc5 = dagnn.Conv('size',[1 1 1024 5],'pad',0,'stride',1,'hasBias',true);
...
net.params(27).value = single(0.05*randn(1,1,1024,5));
net.params(28).value = single(zeros(1,5)');
%[opts, varargin] = vl_argparse(opts, varargin) ;
opts.imdbPath = [];
opts.networkType = 'dagnn' ;
opts.train = struct() ;
opts.train.expDir = fullfile('data','exp_regression_avgmax');
opts.train.gpus = [10];
